...
export default async function AdminHomePage() {
  ...
  const [admin, menu, tomorrowMenu, mealTypes, orders, cancelledToday, ledger] = await prisma.$transaction([
    ...
    prisma.ledgerEntry.groupBy({ by: ["customerId"], where: { businessId: principal.businessId }, _sum: { amountMinor: true }, orderBy: { customerId: "asc" } }),
  ]);
  ...
  return <AppShell businessName={admin.business.name} adminName={admin.name}><div className="grid gap-5">
    ...
    <section className="paper-panel rounded-2xl p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="utility-type text-[10px] font-bold uppercase tracking-[.1em] text-[var(--muted)]">Today’s menu</p><h2 className="mt-1 text-2xl font-black">Kitchen menu</h2>{menu && menu.status !== "PUBLISHED" ? <p className="mt-1 text-xs font-bold uppercase text-[var(--danger)]">{menu.status} — not visible to customers</p> : null}</div><Link className="text-sm font-bold text-[var(--saffron-deep)]" href={`/admin/menus?date=${today}`}>Manage menu →</Link></div><div className="mt-4 grid gap-3 sm:grid-cols-3">{menu?.items.length ? menu.items.map((item) => <article key={item.mealType.id} className="rounded-xl border border-[var(--line)] bg-white/70 p-4"><p className="text-xs font-bold uppercase text-[var(--muted)]">{item.mealType.name}</p><p className="mt-2 font-black">{item.name}</p></article>) : <p className="text-sm text-[var(--muted)]">No menu has been created for today. <Link href={`/admin/menus?date=${today}`} className="font-bold text-[var(--saffron-deep)]">Create it now.</Link></p>}</div></section>
