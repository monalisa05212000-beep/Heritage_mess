...
import { deactivateCustomerAccess } from "@/lib/domain/customers";
...
export async function POST(request: Request, context: { params: Promise<{ customerId: string }> }) {
  ...
  try {
    ...
    await deactivateCustomerAccess(prisma, { role: "ADMIN", businessId: principal.businessId, userId: principal.userId }, customerId, parsed.data.idempotencyKey);
    ...
    return NextResponse.json({ error: "Customer could not be deactivated." }, { status: 500 });
  }
}
